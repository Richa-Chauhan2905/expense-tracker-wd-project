...
let currentCurrencySymbol = localStorage.getItem("currentCurrencySymbol") || "$";
...
function loadUserData() {
    if (currentUser && currentUser.id) {
        ...
        console.log(`Loaded ${expenses.length} expenses for user ${currentUser.id}`);
    }
}
...
function setupEventListeners() {
    ...
    if (cancelBudgetBtn) cancelBudgetBtn.addEventListener("click", hideBudgetForm);
    ...
    if (confirmDelete) confirmDelete.addEventListener("click", confirmDeleteExpense);
    if (cancelDelete) cancelDelete.addEventListener("click", hideDeleteExpenseModal);
...
function renderExpensesList() {
    ...
    if (filterCategory && filterCategory.value) {
        filteredExpenses = filteredExpenses.filter((e) => e.category === filterCategory.value);
    }
    ...
    expensesList.innerHTML = `
        <div class="expenses-summary" style="padding: 1rem 1.5rem; border-bottom: 1px solid var(--border-color);">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="color: var(--text-secondary);">Total: ${filteredExpenses.length} expenses</span>
        ...
        ${filteredExpenses
            ...
            <div class="expense-item">
                ...
                <div class="expense-actions">
                    <button class="btn btn-sm btn-secondary" onclick="editExpense(${expense.id})" title="Edit Expense">
                        ✏️ Edit
                    </button>
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            return `${label}: ${currentCurrencySymbol}${value.toFixed(2)} (${percentage}%)`;
                        }
                    }
                }
            },
...
function updateComparisonStats(currentExpenses, previousExpenses) {
    ...
    if (currentMonthStat) {
        currentMonthStat.textContent = `${currentCurrencySymbol}${currentTotal.toFixed(2)}`;
    }
    
    if (previousMonthStat) {
        previousMonthStat.textContent = `${currentCurrencySymbol}${previousTotal.toFixed(2)}`;
    }
    
    if (differenceStat) {
        const isPositive = difference >= 0;
        differenceStat.textContent = `${isPositive ? "+" : ""}${currentCurrencySymbol}${Math.abs(difference).toFixed(2)}`;
...
function handleEditExpense(e) {
    ...
    const expenseIndex = expenses.findIndex((e) => e.id === currentEditingExpenseId);
...
function updateDashboard() {
    ...
    if (thisMonthTotalEl) thisMonthTotalEl.textContent = `${currentCurrencySymbol}${thisMonthTotal.toFixed(2)}`;
    if (lastMonthTotalEl) lastMonthTotalEl.textContent = `${currentCurrencySymbol}${lastMonthTotal.toFixed(2)}`;
    if (monthlyBudgetEl) monthlyBudgetEl.textContent = `${currentCurrencySymbol}${monthlyBudget.toFixed(2)}`;
